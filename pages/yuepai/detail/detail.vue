<template>
	...
	<view class="wrap">
			...
			<view class="btn">
				<u-grid :col="3" :border='border'>
					...
					<u-grid-item>
						...
						<view class="grid-text">{{model.pchengben}}</view>
						<!-- <view class="grid-text">{{model.money}}</view> -->
					</u-grid-item>
				</u-grid>
			</view>
			<u-line color="primary" />
		...
		<view class="content">
			...
			<view style="color: gray;" v-if="model.ptime!=''">上班时间：{{model.ptime}}</view>
			<view style="color: gray;" v-if="model.money!=0">需要人数：{{model.money}}人</view>
			...
			<view v-if="length == 1"
				...
				<image @click="previewImage(index1,model.urlsList)" v-for="(res, index1) in model.urlsList" :src="res"
					mode="aspectFill" style="width: 100%;"></image>
			</view>

			<view v-if="length == 2 || length == 4"
				style="padding: 20rpx;display: flex;justify-content: space-between;flex-wrap: wrap;">
				<image @click="previewImage(index1,model.urlsList)" v-for="(res, index1) in model.urlsList" :src="res"
					mode="aspectFill" style="width:334rpx;height:334rpx;margin-bottom: 10rpx;"></image>
			</view>
			<view v-if="length == 3 || length == 6 || length == 7"
				style="padding: 20rpx;display: flex;justify-content: space-between;flex-wrap: wrap;">
				<image @click="previewImage(index1,model.urlsList)" v-for="(res, index1) in model.urlsList" :src="res"
			...
			<view class="tags">
				<u-tag class="tag" v-for="(res1,index1) in model.ptagsList" shape="circle" :text="res1" mode="plain" />
			</view>
		...
		<view class="navigation">
			<view class="right">
				<view v-if="model.b==null" class="cart btn u-line-1" @click="$noMultipleClicks(getSc,id)">收藏</view>
				<view v-else class="buy btn u-line-1" @click="$noMultipleClicks(unSc,id)">已收藏</view>
	...
	export default {
		...
		methods: {
			getSc(res) {
				let y_id = res
				api.request({
					url: '/shoucangY/addShoucangY',
					method: 'GET',
					data: {
						yId: y_id, //约课主键id
						userId: this.m_id //自己的id
					},
				}).then(res => {
					if (res.data.data == 1) {
						this.init();
				...
				uni.getStorage({
					...
					success: (res) => {
						...
						api.request({
							...
							this.length = res1.data.urlsList.length
							api.request({
			...
			focus(res) {
				api.request({
					...
					data: {
						uid: this.m_id,
						fid: this.model.pid, //约课文章用户id
						fava: this.model.pava,
						fname: this.model.pname,
						fgender: this.model.pgender,
						uava: this.m_ava, //自己的头像
						uname: this.m_name,
